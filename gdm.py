...
class GDM():
    ...
    def build_discriminator(self, state, action, is_training=False, update_collection=None, lookahead=1, ngf=64):
        ...
        output = lib.nn.linear.Linear(
            'Dence1', 16 * 25 + self.num_actions*lookahead, 18, output, initializer=self.initializer, weight_norm_scale=self.disc_weight_norm_scale, spectral_norm=True, pytorch_biases=True, update_collection=update_collection)
        output = tf.nn.leaky_relu(output, 0.2)
        # (None, 18)
        ...
        output = lib.nn.linear.Linear(
            'Dence2', 18 + self.num_actions*lookahead, 1, output, initializer=self.initializer, weight_norm_scale=self.disc_weight_norm_scale, spectral_norm=True, pytorch_biases=True, update_collection=update_collection)
        # (None, 3*lookahead)
