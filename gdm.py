...
class GDM():
    ...
    def __init__(self, session, config, num_actions=18):
        ...
        with tf.variable_scope('gdm'):
            self.predict_state = self.build_gdm(
                self.pre_state, self.action, self.is_training)
        with tf.name_scope('opt'):
            self.gdm_train_op, self.disc_train_op, self.gdm_summary, self.disc_summary = self.build_training_op(
                self.pre_state, self.post_state, self.predict_state, self.action, self.is_training)
    ...
    def build_discriminator(self, state, action, is_training=False, update_collection=None, ngf=64):
        ...
        output = lib.nn.linear.Linear(
            'Dence1', 400 + self.num_actions*self.lookahead, 18, output, weight_norm_scale=0.1, spectral_norm=True, update_collection=update_collection)
        output = tf.nn.leaky_relu(output, -0.2)
        # (None, 18)
        ...
        output = lib.nn.linear.Linear(
            'Dence2', 18 + self.num_actions*self.lookahead, 1, output, weight_norm_scale=0.1, spectral_norm=True, update_collection=update_collection)
        # (None, 3*lookahead)
    ...
    def build_training_op(self, pre_state, post_state, predict_state, action, is_training):
        ...
        with tf.name_scope('disc_fake'):
            with tf.variable_scope('discriminator'):
                disc_fake = self.build_discriminator(
                    fake_state, action, is_training, update_collection=None)

        with tf.name_scope('disc_real'):
            with tf.variable_scope('discriminator', reuse=True):
                disc_real = self.build_discriminator(
                    real_state, action, is_training, update_collection='NO_OPS')
        ...
        with tf.name_scope('loss'):
            ...
            with tf.name_scope('weight_decay'):
                gdm_l2_loss = tf.losses.get_regularization_loss(
                    scope='gdm', name='gdm_l2_loss')
                disc_l2_loss = tf.losses.get_regularization_loss(
                    scope='discriminator', name='disc_l2_loss')

                gdm_loss += gdm_l2_loss
                disc_loss += disc_l2_loss
